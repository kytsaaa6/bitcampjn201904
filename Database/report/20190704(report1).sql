...
;
select job, deptno as 'dno',
		group_concat('$', format(ifnull(CASE deptno WHEN 10 THEN sum(sal) end, 0), 0),
        format(ifnull(CASE deptno WHEN 20 THEN sum(sal) end, 0), 0),
        format(ifnull(CASE deptno WHEN 30 THEN sum(sal) end, 0), 0)),
        format(sum(sal),0)
from emp
group by deptno,job order by deptno asc;
;